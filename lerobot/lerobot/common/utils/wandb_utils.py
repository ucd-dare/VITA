...
def cfg_to_group(cfg: TrainPipelineConfig, return_list: bool = False) -> list[str] | str:
    ...
    lst = [
        ...
        # f"dataset:{cfg.dataset.repo_id if hasattr(cfg, 'dataset') else 'none'}",
        f"seed:{cfg.seed}",
    ]
...
def get_wandb_run_id_from_filesystem(log_dir: Path) -> str:
    ...
    if len(paths) != 1:
        ...
        # look for wandb/run-*-{wandb_run_id}
        paths = glob(str(log_dir / "wandb/run-*"))
        if len(paths) == 0:
            raise RuntimeError("Couldn't get the previous WandB run ID for run resumption.")
        
        wandb_run_id = paths[0].split("/")[-1].split("-")[-1]
...
class WandBLogger:
    ...
    def __init__(self, cfg: TrainPipelineConfig):
        ...
        if cfg.wandb.run_id is not None:
            ...
            print(colored(f"Resuming WandB run {wandb_run_id}.", "blue", attrs=["bold"]))
        ...
        wandb.init(
            ...
            # dir=self.log_dir, THIS STOPS VIDEOS FROM BEING LOGGED
    ...
    def log_dict(self, d: dict, step: int, mode: str = "train"):
        # if mode not in {"train", "eval", "visualize"}:
    ...
    def log_video(self, video_path: str, step: int, key: str):
        # if mode not in {"train", "eval", "visualize"}:
        #     raise ValueError(mode)

        wandb_video = self._wandb.Video(video_path, fps=self.fps, format="mp4")
        self._wandb.log({key: wandb_video}, step=step)
